...
class LegJoints(NamedTuple):
    thigh: pm.nt.Joint
    thigh_twist: pm.nt.Joint
    calf: pm.nt.Joint
    calf_twist: pm.nt.Joint
    foot: pm.nt.Joint
...
class LegRigComponents(NamedTuple):
    ...
    controller_fk_thigh_twist: pm.nt.Transform
    controller_fk_calf: pm.nt.Transform
    controller_fk_calf_twist: pm.nt.Transform
...
class LegRig(ue_rig.RigModule):
    ...
    def build_rig(self):
        self.rig_components = build_leg_rig(self.bind_joints)
...
def build_leg_rig(bind_leg_joints: LegJoints, module_name='leg', module_group=None):
    ...
    limb_bind_joints = ue_rig.LimbJoints(*bind_leg_joints)
    limb_ik_joints = ue_rig.LimbJoints(*ik_joints_struct)
    limb_fk_joints = ue_rig.LimbJoints(*fk_joints_struct)
    (fk_controls, ik_thigh_loc_ori, ik_thigh_ctr, ik_foot_ctr, pole_vector_ctr,
     ik_handle, upper_twist_ctr, lower_twist_ctr, switch_ctr, parent_blend_attr) = ue_rig.rig_limb(
        limb_bind_joints, limb_fk_joints, limb_ik_joints, controls_group, ik_group, controls_group, module_name, side, 'thigh', 'foot')

    leg_rig_struct = LegRigComponents(
        module_group, bind_leg_joints, ik_joints_struct, fk_joints_struct, ik_thigh_ctr, ik_foot_ctr,
        fk_controls[0], upper_twist_ctr, fk_controls[1], lower_twist_ctr, fk_controls[2],
...
def duplicate_leg_joints(leg_joints: LegJoints, prefix, parent=None):
    ...
    rigutils.safe_parent(new_leg_joints_struct.calf, new_leg_joints_struct.calf_twist)
    rigutils.safe_parent(new_leg_joints_struct.thigh, new_leg_joints_struct.calf)
    rigutils.safe_parent(new_leg_joints_struct.thigh, new_leg_joints_struct.thigh_twist)
    if parent:
...
def get_leg_joints_from_scene(side_suffix):
    ...
    thigh_twist = ue_rig.get_joint_by_name(ue_rig.JOINT_NAME_THIGHTWIST, side_suffix)
    calf = ue_rig.get_joint_by_name(ue_rig.JOINT_NAME_CALF, side_suffix)
    calf_twist = ue_rig.get_joint_by_name(ue_rig.JOINT_NAME_CALFTWIST, side_suffix)
    foot = ue_rig.get_joint_by_name(ue_rig.JOINT_NAME_FOOT, side_suffix)
    return LegJoints(thigh, thigh_twist, calf, calf_twist, foot)
...
def leg_constrain_rig_to_bind_skeleton(leg_rig_components: LegRigComponents):
    ...
    for parent, child in [
        (leg_rig_components.joints_bind.thigh_twist, leg_rig_components.controller_fk_thigh_twist),
        (leg_rig_components.controller_fk_thigh_twist, leg_rig_components.controller_fk_calf_twist)]:
...
def leg_constrain_bind_joints_to_rig(leg_rig_components: LegRigComponents):
    rig_constraints = []
    rig_constraints.append(pm.parentConstraint(leg_rig_components.controller_fk_calf_twist,
                                               leg_rig_components.joints_bind.calf_twist, maintainOffset=True))
    rig_constraints.append(pm.parentConstraint(leg_rig_components.controller_fk_thigh_twist,
                                               leg_rig_components.joints_bind.thigh_twist, maintainOffset=True))
    bind_joints = [leg_rig_components.joints_bind.thigh,
                   leg_rig_components.joints_bind.calf,
                   leg_rig_components.joints_bind.foot,
                   leg_rig_components.controller_fk_thigh_twist.getParent(),
                   leg_rig_components.controller_fk_calf_twist.getParent()]
    ik_joints = [leg_rig_components.joints_ik.thigh,
                 leg_rig_components.joints_ik.calf,
                 leg_rig_components.joints_ik.foot,
                 leg_rig_components.joints_ik.thigh_twist,
                 leg_rig_components.joints_ik.calf_twist]
    fk_joints = [leg_rig_components.joints_fk.thigh,
                 leg_rig_components.joints_fk.calf,
                 leg_rig_components.joints_fk.foot,
                 leg_rig_components.joints_fk.thigh_twist,
                 leg_rig_components.joints_fk.calf_twist]
...
def leg_get_controllers_to_bake(leg_rig_components: LegRigComponents):
    controllers_to_bake = [leg_rig_components.controller_fk_thigh,
                           leg_rig_components.controller_fk_thigh_twist, leg_rig_components.controller_fk_calf,
                           leg_rig_components.controller_fk_calf_twist, leg_rig_components.controller_fk_foot,
                           leg_rig_components.controller_ik_thigh]
